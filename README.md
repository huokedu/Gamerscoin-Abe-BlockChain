Abe: a free block chain browser for Gamerscoin.
...
This software reads the Gamerscoin block file, transforms and loads the
data into a database, and presents a web interface similar to Gamerscoin
...
Abe works with files created by the original (Satoshi) Gamerscoincoin client.
You will need a copy of the block files (blk0001.dat, blk0002.dat,
etc. in your Gamerscoin directory or its blocks/ subdirectory).  You may
let Abe read the block files while Gamerscoin runs, assuming Gamerscoin only
appends to the file.

Gamerscoin support depends on the ltc_scrypt module
...
* named arguments as a JSON object, e.g.:
  connect-args = { "database": "abe", "password": "passwd" }

* positional arguments as a JSON array, e.g.:
  connect-args = ["abe", "abe", "passwd"]
...
the main GMC block chain as of 2013.  However, if you use a persistent
...
To display Gamerscoin Bitcoin Namecoin, NovaCoin, or any block chain with data somewhere
other than the default Bitcoin directory, specify "datadir" in
abe.conf, e.g.:

    datadir = /home/gamerscoin/.gamerscoin

The datadir directive can include a new chain's basic configuration,
e.g.:

    datadir += [{
            "dirname": "/home/gamerscoin/testnet",
            "chain":   "GamersCoin",
            "code3":   "GMC",
            "address_version": "o" }]
